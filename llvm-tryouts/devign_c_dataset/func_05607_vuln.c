	...
	if(s->aspect_ratio_info == EXTENDET_PAR){
        ...
        if (s->shape != BIN_ONLY_SHAPE) {
            ...
            if (vo_ver_id == 1) {
                ...
                s->vol_sprite_usage = get_bits1(&s->gb); 

            } else {

                s->vol_sprite_usage = get_bits(&s->gb, 2); 
            ...
            if (get_bits1(&s->gb) == 1) {   
                ...
                s->quant_precision = get_bits(&s->gb, 4); 
	...
	s->no_rounding = 0;
     ...
     if (s->shape != RECT_SHAPE) {
         ...
         if (s->vol_sprite_usage != 1 || s->pict_type != I_TYPE) {
             ...
             hor_spat_ref = get_bits(&s->gb, 13); 

             skip_bits1(&s->gb);   

             ver_spat_ref = get_bits(&s->gb, 13); 
